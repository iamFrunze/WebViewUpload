package testchaf.apptestest.screens.test;
...
public class MainActivity extends Activity
    ...
    public void showOptions() {
        ...
        alertDialog.setTitle("Пожалуйста, выберите операцию");
        // gallery, camera.
        String[] options = {"Фотоальбом", "Фотографировать"};
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (which == 0) {
                            if (PermissionUtil.isOverMarshmallow()) {
                                if (!PermissionUtil.isPermissionValid(MainActivity.this, Manifest.permission.READ_EXTERNAL_STORAGE)) {
                                    Toast.makeText(MainActivity.this,
                                            "1Пожалуйста, перейдите в «Настройки», чтобы разрешить доступ к мультимедийным изображениям для этого приложения.",
                            ...
                            try {
                                ...
                                e.printStackTrace();
                                Toast.makeText(MainActivity.this,
                                        "2Пожалуйста, перейдите в «Настройки», чтобы разрешить доступ к мультимедийным изображениям для этого приложения.",
                                        Toast.LENGTH_SHORT).show();
                                restoreUploadMsg();
                            }
                            ...
                            if (PermissionUtil.isOverMarshmallow()) {
                                if (!PermissionUtil.isPermissionValid(MainActivity.this, Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                                    Toast.makeText(MainActivity.this,
                                            "3Пожалуйста, перейдите в «Настройки», чтобы разрешить доступ к мультимедийным изображениям для этого приложения.",
                                            Toast.LENGTH_SHORT).show();
                                ...
                                if (!PermissionUtil.isPermissionValid(MainActivity.this, Manifest.permission.CAMERA)) {
                                    Toast.makeText(MainActivity.this,
                                            "4Пожалуйста, перейдите в «Настройки», чтобы разрешить разрешение камеры в этом приложении.",
                                            Toast.LENGTH_SHORT).show();
